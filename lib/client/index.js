...
function SocketPouch(opts, callback) {
  ...
  if(typeof URL !== 'undefined') {
    ...
    if(!parsedUrl.pathname.match(new RegExp('\\b' + opts.name + '\\b'))) {
      parsedUrl.pathname = (parsedUrl.pathname.endsWith('/') ? '': '/') + opts.name;
      opts.url = parsedUrl.toString();
    }
  } else {
    opts.url = opts.url + '/' + opts.name;
  }
  ...
  function useExistingSocket() {
    ...
    if (instance._socketId) {
      ...
      api._socket.once('connect', function () {
  ...
  function createNewSocket() {
    ...
    socket.on('reconnect', function () {
      log('calling createDatabase on reconnect, jic.');
      sendMessage('createDatabase', [serverOpts], function(err) {
        log('createDatabase on reconnect called', err);
        Object.keys(api._changesListeners).forEach(function(k) {
          log('recreating listener', k);
          api._changesListeners[k].recreate();
        })
        api.taskqueue.isReady = true;
        api.taskqueue.execute();
      });
    });

    socket.on('disconnect', function () {
      api.taskqueue.isReady = false;
    });
  ...
  api._changes = function (opts) {
    ...
    if (opts.continuous) {
      ...
      function cancel() {
        ...
        delete api._changesListeners[messageId];
      }
      return {
        cancel: cancel
      };
